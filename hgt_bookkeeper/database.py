...
class Database:
    ...
    def insert_transaction(self, txn: Transaction) -> bool:
        ...
        try:
            ...
            """, (
                txn.id, txn.source, txn.source_id, txn.source_type, txn.date,
                txn.type, txn.income_category, txn.description, txn.gross,
                txn.fees, txn.net, txn.currency, txn.available_on, txn.payout_id,
                txn.created_at, txn.metadata
            ))
            conn.commit()
            return True
    ...
    def insert_tax_calculation(self, calc: TaxCalculation):
        ...
        """, (
            calc.transaction_id, calc.fica_employee, calc.fica_employer,
            calc.federal, calc.state, calc.total, calc.calculated_at
        ))
        conn.commit()
    ...
    def log_export(
        ...
        conn.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        """, (exporter, start_date, end_date, transaction_count, datetime.now().isoformat(), output_file))
        conn.commit()
    ...
    def get_summary(self) -> dict:
        ...
        cursor = conn.execute("SELECT COUNT(*) FROM transactions WHERE type = 'revenue'")
        revenue_count = cursor.fetchone()[0]
        
        cursor = conn.execute("SELECT COUNT(*) FROM transactions WHERE type = 'payout'")
        ...
        cursor = conn.execute("SELECT COUNT(*) FROM transactions WHERE type = 'revenue' AND payout_id IS NULL")
